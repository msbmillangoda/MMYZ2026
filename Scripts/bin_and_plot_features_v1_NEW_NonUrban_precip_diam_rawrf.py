...
# =============================================
plot_dir = Path("Plot_Feature_Counts_10km_v1_NEW_NonUrban_precip_diam_rawrf")
plot_dir.mkdir(exist_ok=True)

# Input CSVs (CLUSTERS 2 and 3; NOTE: your original had cluster_2 twice)
files = [
...
# -------------------------------
# Outer domain (black box) and inner exclusion (green box)
# -------------------------------
black_box = Polygon([
...
# Region of interest = black box minus green box
outside_region = black_box.difference(green_box).buffer(0)  # buffer(0) to clean geometry
...
# Default smoothing (used in plots + rainfall fields)
DEFAULT_SMOOTH_SIGMA = 1.5

# =============================================
# Distance Calculation from Middle Line (mask to OUTSIDE region)
# =============================================
def calculate_distance(lat, lon):
    """
    Signed geodesic distance (km) from (lat, lon) to middle_line.
    Positive/negative via cross product sign.
    Returns NaN if the point is outside the OUTSIDE-OF-GREEN region.
    """
    point = Point(lon, lat)
    if not outside_region.contains(point):
        return np.nan
...
def bin_rainfall_by_distance_and_hour(rainfall_da, region_poly, bin_edges, bin_labels, smooth_sigma=DEFAULT_SMOOTH_SIGMA):
    ...
    bin_idx = np.digitize(dist_flat, bin_edges) - 1  # 0..n_bins-1
    ...
    # valid points are those inside region + inside bin range
...
# Signed distance (km) from middle_line; NaN if outside OUTSIDE region
...
# =============================================
# Create 10km x 10km Grid Aligned to Middle Line (clipped to OUTSIDE region)
# =============================================
# Project outside region and middle line to metric CRS
outside_proj = gpd.GeoSeries([outside_region], crs="EPSG:4326").to_crs(epsg=3857)[0]
line_proj = gpd.GeoSeries([middle_line], crs="EPSG:4326").to_crs(epsg=3857)[0]

# Rotation angle (degrees) of the middle line
x0, y0 = line_proj.coords[0]
x1, y1 = line_proj.coords[-1]
angle_deg = np.degrees(np.arctan2((y1 - y0), (x1 - x0)))

# Envelope in meters
xmin, ymin, xmax, ymax = outside_proj.bounds

# Build 10x10 km squares aligned with the line, keep those intersecting OUTSIDE region
grid_cells = []
x = xmin
while x < xmax:
    y = ymin
    while y < ymax:
        ...
        if outside_proj.intersects(cell):
...
# Signed distance (km) from middle line in projected CRS
...
# Distance bands (labels are midpoints)
...
# -------------------------------------------------
# Aggregate counts + mean_precip + diameter_km
# -------------------------------------------------
binned = (
...
# Average per day in that (hour, distance_bin)
binned["count_per_day"] = binned["raw_count"] / n_days

# Number of contributing cells per distance band (within OUTSIDE region)
grid_bins = pd.cut(
...
# Convenience: avg cells/day per 10x10 km cell in the band
binned["per_cell_rate_per_day"] = (
    binned["count_per_day"] / binned["n_cells"]
).replace([np.inf, -np.inf], np.nan)

# Save detailed table
csv_path = plot_dir / "Feature_Counts_and_Densities_by_Hour_and_Binned_Distance_OutsideGreen.csv"
...
# Plots (OUTSIDE region)
plot_metric(
    pivot_count_per_day,
    title="Smoothed Average Count per Day (Non-Urban: Outside Green Box)",
    cbar_label="Average Count per Day (cells / day)",
    outfile=plot_dir / "Smoothed_Count_per_Day_OutsideGreen.png",
    y_limits=(-10, 200),
...
plot_metric(
    pivot_density_100km2,
    title="Smoothed Areal Feature Density (Non-Urban: Outside Green Box)",
    cbar_label="Feature Density (cells / 100 km^2 / day)",
    outfile=plot_dir / "Smoothed_Areal_Density_per_100km2_per_Day_OutsideGreen.png",
    y_limits=(-10, 200),
    smooth_sigma=DEFAULT_SMOOTH_SIGMA
)
...
# Equal-weight mean across clusters
rainfall_all = (rf0 + rf1 + rf2 + rf3) / 4.0

# Bin rainfall by distance for each strip
...
_, raw_rainfall_nonurban_binned, raw_rainfall_nonurban_binned_smoothed = bin_rainfall_by_distance_and_hour(
    rainfall_all, outside_region, bin_edges, bin_labels, smooth_sigma=DEFAULT_SMOOTH_SIGMA
)

# =============================================
# Save unsmoothed metrics to NetCDF (NON-URBAN strip) + include URBAN/NON-URBAN rainfall fields
# =============================================
...
ds = xr.Dataset(
    {
        ...
        # NEW: rainfall binned fields (ALL clusters, split into strips)
        ...
        # NEW: smoothed rainfall binned fields (useful for contour overlays)
    ...
    attrs={
        "title": "Non-urban feature counts/densities and rainfall (urban vs non-urban strips) by hour and distance",
        "description": (
            "count_per_day, areal_density_per_100km2_day, mean_precip, diameter_km computed from features "
            "restricted to NON-URBAN strip (black_box minus green_box). "
            "Rainfall fields are computed from CMORPH cluster0-3 composites (equal-weight mean across clusters) "
            "and binned by signed distance within URBAN (green_box) and NON-URBAN (black_box minus green_box) strips."
        ),
...
netcdf_path = plot_dir / "Counts_and_ArealDensities_OutsideGreen_Strong.nc"
