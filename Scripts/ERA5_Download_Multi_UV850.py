...
# Safer for CDS and avoids invalid dates (e.g., June/Sep 31)
CHUNK_BY_MONTH = True

# Use the newer keys youe been using
COMMON = {
...
for year in YEARS:
    if CHUNK_BY_MONTH:
        for month in MONTHS:
            ...
            out_name = f"ERA5_500hPa_UV_{year}_{month}.nc"
        ...
        # Single file per year (requests all MJJAS days; not as safe re: 30/31-day months)
        request = {
            **COMMON,
            "year": [str(year)],
            "month": MONTHS,
            "day": [f"{d:02d}" for d in range(1, 32)],  # may fail for 30-day months
        }

        out_name = f"ERA5_850hPa_UV_{year}.nc"
        out_path = os.path.join(OUT_DIR, out_name)
